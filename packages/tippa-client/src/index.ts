...
import type { u32, i32, u64, i64, u128, i128, u256, i256, Option, Timepoint, Duration } from "@stellar/stellar-sdk/contract"
...
export type DataKey =
  | { tag: "Owner"; values: readonly [string] }
  | { tag: "Rules"; values: readonly [string] }
  | { tag: "Pool"; values: readonly [string, string] }
  | { tag: "TotalReceived"; values: readonly [string, string] }
  | { tag: "TotalReceivedFromOthers"; values: readonly [string, string] }
  | { tag: "Unclaimed"; values: readonly [string, string] }
  | { tag: "DonorToUser"; values: readonly [DonorKey] }
  | { tag: "DonorTotal"; values: readonly [string, string] }
  | { tag: "TotalForwarded"; values: readonly [string, string] }
  | { tag: "GrandTotal"; values: readonly [string] }
  | { tag: "PaidTo"; values: readonly [string, string] }
...
export interface Client {
   ...
   */
  claim: (
    { caller, username, asset, to }: { caller: string; username: string; asset: string; to: Option<string> },
  ...
  donate: (
    {
      ...
      amount,
      donor_override,
    }: { caller: string; username: string; asset: string; amount: i128; donor_override: Option<string> },
    options?: MethodOptions,
   ...
   */
  set_rules: (
    { caller, username, rules }: { caller: string; username: string; rules: Map<string, u32> },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<Result<void>>>
   ...
   */
  distribute: (
    { username, asset, min_distribution }: { username: string; asset: string; min_distribution: i128 },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<Result<void>>>
   ...
   */
  get_donor_to_user: (
    { donor, username, asset }: { donor: string; username: string; asset: string },
    options?: MethodOptions,
   ...
   */
  transfer_ownership: (
    { caller, username, new_owner }: { caller: string; username: string; new_owner: string },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<Result<void>>>
  ...
  distribute_and_claim: (
    {
      ...
      to,
      min_distribution,
    }: { caller: string; username: string; asset: string; to: Option<string>; min_distribution: i128 },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<Result<i128>>>
   ...
   */
  get_total_received_from_others: (
    { username, asset }: { username: string; asset: string },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<i128>>
}
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([
        "",
